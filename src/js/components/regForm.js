...
const changeModalRegContent = (state, content) => {
  ...
  const currentState = statesFragment
    .querySelector(`.reg-modal__state[data-state="${state}"]`)
    ?.cloneNode(true);
  content.innerHTML = "";
...
const checkCode = () => {
  const codeInput = document.querySelector(".reg-modal__input.code");
...
async function handleRegSubmit(event) {
  ...
  try {
    ...
    if (status === "ok") {
      ...
      const regHomeContent = currentForm.querySelector(
      ...
      initCustomMasks();
      showInfoModal(
        "Регистрация прошла успешно. На указанный вами номер телефона поступит ссылка на билет."
      );
    } else {
